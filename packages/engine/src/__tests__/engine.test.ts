...
vi.mock('@open-flow/nodes', () => ({
  runLLMPrompt: (input: any, config: any) => mockLLMRun(input, config),
  runTextTransform: (input: any, config: any) => mockTextTransformRun(input, config),
  runSQLiteStorage: (input: any, config: any) => mockSQLiteRun(input, config),
  runMCPTool: vi.fn(),
  runHTTPWebhook: vi.fn(),
  runCronTrigger: vi.fn(),
  runWebhookTrigger: vi.fn(),
  runFileTrigger: vi.fn(),
  runEmail: vi.fn(),
  runVisionOCR: vi.fn(),
  runVectorStore: vi.fn(),
  runVectorRetrieve: vi.fn(),
  runBranch: vi.fn(),
}));

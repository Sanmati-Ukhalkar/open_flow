...
vi.mock('@open-flow/db', async (importOriginal) => {
...
import { db, DatabaseWrapper } from '@open-flow/db';
...
import { seedTemplates } from '@open-flow/db';
...
describe('End-to-End Workflow Templates Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    await dbRun(`INSERT OR IGNORE INTO credentials (id, user_id, org_id, provider, encrypted_key) VALUES (?, ?, ?, ?, ?)`, ['c1', userId, orgId, 'openai', 'key1']);
    await dbRun(`INSERT OR IGNORE INTO credentials (id, user_id, org_id, provider, encrypted_key) VALUES (?, ?, ?, ?, ?)`, ['c2', userId, orgId, 'groq', 'key2']);
  ...
  beforeEach(async () => {
    await dbRun('DELETE FROM runs');
    await dbRun('DELETE FROM run_node_results');
  });
  ...
  it('Template 1: Document Extraction Pipeline should run successfully', async () => {
    ...
    if (run.status !== 'success') console.log('RUN 1 ERROR:', run.error);

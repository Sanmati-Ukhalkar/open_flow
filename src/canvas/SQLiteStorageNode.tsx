...
import { Database, Loader2, CheckCircle2, AlertCircle, XCircle, AlertTriangle } from 'lucide-react';
...
export const SQLiteStorageNode = ({ data, selected }: { data: SQLiteStorageNodeData; selected: boolean }) => {
  const statusColors = {
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-zinc-800 pb-2 mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-zinc-100">SQLite Storage</span>
        </div>
        ...
        <div className="flex items-center">
          ...
          {data.status === 'skipped' && (
            <XCircle className="w-4 h-4 text-zinc-655" />
          )}
          {data.status === 'idle' && (
